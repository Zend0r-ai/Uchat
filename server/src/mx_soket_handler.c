...
static void mx_get_message_history(int fd_from/*, int *connected_users*/);
...
void mx_socket_handler(int client_sock, int *connected_users) {
    ...
    ssize_t rc = read(client_sock, buffer, BUFFER_SIZE-1);
    if (rc > 0) {
        jobj = json_tokener_parse(buffer);
        if (jobj != NULL) {
            //printf("%p\n", (void*)jobj); // DEBUG line
...
void mx_parse_message(json_object *jobj, int *connected_users, int fd_from) {
    ...
    if (strcmp(type, "new_message") == 0) {
        ...
        mx_get_message_history(fd_from/*, connected_users*/);
    }
...
static void mx_get_message_history(int fd_from/*, int *connected_users*/) {
    int fact_depth = 0;
    t_message *history = db_get_history(&db, HISTORY_DEPTH, &fact_depth/*, connected_users[fd_from]*/);
...
static void mx_message_handler(json_object *jobj, int *connected_users, int fd_from, char *msg_type, bool (*msg_handler)(sqlite3**, t_message)) {
    ...
    if (message_handled) {
        json_str = json_pack_message(new_message, 0, msg_type);
        printf("json object = %s\n", json_str); // DEBUG line
        write_to_socket(json_str, connected_users, fd_from, 1);
    }
    else {
        json_str = json_pack_message(new_message, 1, msg_type);
        printf("json object = %s\n", json_str); // DEBUG line
        write_to_socket(json_str, NULL, fd_from, 0);
    }
    free(json_str);
