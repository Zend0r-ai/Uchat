...
// static void mx_new_message(json_object *jobj, int *connected_users, int fd_from);
// static void mx_delete_message(json_object *jobj, int *connected_users, int fd_from);
// static void mx_update_message(json_object *jobj, int *connected_users, int fd_from);
...
static void mx_message_handler(json_object *jobj, int *connected_users, int fd_from, char *msg_type, msg_handler handler);
...
void mx_parse_message(json_object *jobj, int *connected_users, int fd_from) {
    ...
    if (strcmp(type, "new_message") == 0) {
        ...
        //mx_update_message(jobj, connected_users, fd_from, "update_message_back");
...
static void mx_get_message_history(int fd_from) {
    ...
    //     char buf;
    //     read(fd_from, &buf, 1); // wait for signal 'READY_TO_READ' from client
...
// static void mx_new_message(json_object *jobj, int *connected_users, int fd_from, char *msg_type) {
...
// static void mx_delete_message(json_object *jobj, int *connected_users, int fd_from, char *msg_type) {
//     t_message *new_message = json_unpack_message(jobj);

//     bool message_added = db_delete_message(&db, new_message);
...
// static void mx_update_message(json_object *jobj, int *connected_users, int fd_from, char *msg_type) {
//     t_message *new_message = json_unpack_message(jobj);

//     bool message_added = db_update_message(&db, new_message);
...
static char *json_pack_message(t_message *msg, int err_code, char *msg_type) {
    ...
    return *json_str = (char *)json_object_to_json_string(jback);
}

static void mx_message_handler(json_object *jobj, int *connected_users, int fd_from, char *msg_type, msg_handler handler) {
     t_message *new_message = json_unpack_message(jobj);

    bool message_handled = handler(&db, new_message);
